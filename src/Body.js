...
function Body({ spotify }) {
  const [{ token, selectedPlaylistId, selectedPlaylist}, dispatch]= useDataLayerValue();
  useEffect(()=>{
  ...
  const playTrack = async (id,name,artists,image,context_uri,track_number) => {
    const response = await axios.put(`https://api.spotify.com/v1/me/player/play`, { context_uri, offset: {position: track_number-1}, position_ms:0}, {
      headers: {
        Authorization: "Bearer " + token,
        "Content-Type": "application/json",
      },
